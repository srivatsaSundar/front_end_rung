...
export function EditMenu() {
  ...
  const handleDeleteAddOn = (name) => {
    axios.delete(`https://backend-rung.onrender.com/delete_add_on/${name}/`)
      .then(response => {
        console.log('Delete Response:', response.data);
      })
      .catch(error => {
...
const handleAddSubmit = (e) => {
  e.preventDefault();
  console.log(addData);
  axios.post('https://backend-rung.onrender.com/add_on/', addData)
    .then(response => {
      console.log('Server Response:', response.data);
    })
    .catch(error => {
  ...
  <div className="mb-3">
    <label>Name:</label>
    <input type="text" className="form-control" value={addData.name} onChange={(e) => handleAddInputChange('name', e.target.value)} />
  </div>
  <div className="mb-3">
    <label>Price:</label>
    <input type="text" className="form-control" value={addData.price} onChange={(e) => handleAddInputChange('price', e.target.value)} />
  </div>
</Modal.Body>
<Modal.Footer>
            ...
            <tbody>
              ...
              add_on.map((item,id) => (
                <tr key={id}>
                  <td>{item.name}</td>
                  <td>{item.price}</td>
                  <td><button onClick={() => handleDeleteAddOn(item.name)}>Delete</button></td>
                </tr>
              ))
              ):(
                <tr>
