...
export function Menu() {
  ...
  // get the restaurant id from the previous page
  useEffect(() => {
    if (rest_id) {
...
// set the menu items into the array and then to the state
...
// add to cart function
  const handleAddToCart = (menuItem: MenuItem, itemIndex: number) => {
    ...
    setTotalCost(
      (prevTotal) => prevTotal + menuItem.itemCards[itemIndex].card.info.rate
    );
  };
  ...
  const handleRemoveFromCart = (itemIndex: number) => {
    setCartItems((prevCartItems) => {
      ...
      const removedItemRate =
        updatedCart[itemIndex].itemCards[0].card.info.rate;
  ...
  return (
    <div>
      ...
      <div className="menu-scroll">
        {/* cart will be shown using modals */}
        <p className="menu-title">Menu</p>
        <button className="btn" onClick={handleToggleCart}>
          View Cart
        </button>
      </div>
      ...
      <div className="menu-container">
            ...
            {menuItem.itemCards.map((itemCard, itemIndex) => (
              <div className="item-card" key={itemIndex}>
                ...
                <button
                  ...
                  onClick={() => handleAddToCart(menuItem, itemIndex)}
                >
                  Add to Cart
                </button>
              </div>
        ...
        {/* modal function to view and close the cart */}
        ...
        {showCart && (
          <Modal isOpen={showCart}>
            <div>
              ...
              <button className="btn" onClick={handleToggleClose}>
                Close
              </button>
            </div>
          </Modal>
