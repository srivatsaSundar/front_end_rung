...
// If you're using Axios, import it
...
export function Order() {
  ...
  const translations =
    ...
    let data = {}; // Define an empty data object
      ...
      axios.post('https://backend-rung.onrender.com/order/', data)
        .then((response) => {
          if (response.status === 200) {
            window.location.href = '/placed'; // If successful, navigate to the next page
          } else {
  ...
  onClick={() => sendOrderToBackend(data)} // Call the function to send the order
>
                                  OK</button>
                              </Link>
                              <button
